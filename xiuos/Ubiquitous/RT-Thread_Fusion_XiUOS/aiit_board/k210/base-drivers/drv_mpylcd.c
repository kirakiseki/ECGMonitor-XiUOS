...
void lcd_show_image(int x, int y, int wide, int height,const rt_uint8_t *buf)
{
...
void lcd_draw_picture(uint16_t x1, uint16_t y1, uint16_t width, uint16_t height, uint32_t *ptr)
{
    ...
    if (lcd_init() != 0)
        return;
    drv_lcd_set_area(lcd,x1, y1, x1 + width, y1 + height);
    drv_lcd_data_word(lcd,ptr, width * height/2);
}

void lcd_draw_16_picture(uint16_t x1, uint16_t y1, uint16_t width, uint16_t height, uint32_t *ptr)
{
    lcd_8080_device_t lcd = (lcd_8080_device_t)dev;
    if (lcd_init() != 0)
        return;
    drv_lcd_set_area(lcd,x1, y1, x1 + width, y1 + height);
    drv_lcd_data_half_word(lcd,ptr, width * height);
